...
let biggerThanLuke = (arr) => {
  ...
  let newArr = arr.filter(function(el){
   ...
   let newArr2 = newArr.map(function(el){
     return el = el.name;
   });
   return newArr2.join(' - ');
}
...
Write a function named detectTicTacToeWin that accepts a two-dimensional array of strings. Each string is guaranteed to be either "X", "O" or an empty string. 
...
Instead of trying to write crazy for loops to automate checking the rows, columns and diagonals consider writing one helper function that accepts three coordinate pairs and checks the values of the array at those locations. 

...
const count = (target, input) => {
  ...
  if(input.length==0) return 0;
       let mappedArr = input.map(function(arr){
     ...
     else if(mappedArr.length!==0){
      let innerArrFilterArr = mappedArr.filter(function(el){
        return el.length>0;
      });
        
        if(innerArrFilterArr.length>0){
          let value = innerArrFilterArr.reduce(function(acc, cur){
...
const totalSum = (input) => {  
 let innerArrSumArr = input.map(function(arr){
    return arr = arr.reduce(function(acc, cur){return acc + cur;},0);
  });
    console.log(innerArrSumArr);
    return innerArrSumArr.reduce(function(acc, cur){
      return acc + cur;
    },0);
  // Solution code here...
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let newArr = input.map(function(arr){
    return arr = arr.filter(function(el){
      return typeof(el) === 'number' && el%5 === 0;
    });
  });
  return newArr.map(function(array){
...
let findMaleAndFemale = (data) => {
  // Solution code here...
   ...
   let arr = newArr.map(function(el){
     return el = el.name;
   });
    return arr.join(' and ');
}
...
let findShortest = (data) => {
  // Solution code here...
 let names = data.map(function(obj){
      return obj = {name: obj.name, height: obj.height};

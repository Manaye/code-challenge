...
const getCourseKeys = (obj) => {
  // Solution code here...
  let propsArray = Object.keys(courseInfo);
...
const hasChildrenValues = (arr, character) => {
  ...
  let vals = Object.values(arr);
    ...
    for(let i = 0; i<vals.length; i++){
      for(let j = 0; j<vals[i].children.length;j++){
        if(vals[i].children[j] === character){
          return true;
        }else{
          return false;
        }
...
const houseSize = (arr) => {
  ...
  arr.forEach(function(el){
      sizes.push({house: el.house, members: 1+(el.spouse?1:0)+(el.children.length?el.children.length:0)});
  });
//   console.log(sizes);
  return sizes;
}
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach(function(el){
      if(el.spouse){
        survivors.push({house: el.house, members: 1+(deceasedSpouses.includes(el.spouse)?0:1)+(el.children.length>0?el.children.length:0)});
      }
       else{
        survivors.push({house: el.house, members: 1+(el.children.length?el.children.length:0)});  
       }
    });
